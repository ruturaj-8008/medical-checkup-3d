...
export const MedicalCanvas: React.FC<MedicalCanvasProps> = ({
  ...
  return (
    <section
      ...
      aria-labelledby="visualization-instructions"
    >
      {/* ThreeJS Canvas */}
        ...
        return (
          ...
          >
            ...
            <button
              ...
              aria-label={`${node.name}: ${node.description}. ${isActive ? 'Selected. Press Enter or Space to deselect.' : 'Press Enter or Space to select.'}`}
              aria-pressed={isActive}
              disabled={isScanning}
            >
              <span className={`absolute w-3 h-3 rounded-full ${isActive ? 'bg-white animate-ping' : 'bg-cyan opacity-75'}`} aria-hidden="true" />
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center pointer-events-none bg-black/35 backdrop-blur-md px-4 py-2 border border-white/5 rounded-full">
        <p id="visualization-instructions" className="text-[10px] text-text-secondary uppercase tracking-widest hud-font">
          Interactive Holographic DNA Model. Move mouse to tilt. Use Tab, Enter, or Space to select nodes.
        </p>
      </div>
    </section>
  );
