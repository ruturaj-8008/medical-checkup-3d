    ...
    <div className="app-container">
      ...
      <header className="col-span-3 border-b border-white/5 bg-black/45 backdrop-blur-md px-6 flex justify-between items-center z-20" aria-label="Aura-3D system status">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] hud-font text-cyan bg-cyan/5 border border-cyan/10 px-3.5 py-1 rounded font-bold" aria-live="polite" aria-label={`System time: ${currentTime || 'loading'}`}>
            {currentTime || 'LOADING...'}
          </span>
        </div>
      </header>
      ...
      <p className="sr-only" aria-live="polite" aria-atomic="true">
        {isScanning ? `Diagnostic scan in progress: ${Math.round(scanProgress)} percent complete.` : showReport ? 'Diagnostic report is ready.' : 'Diagnostic scanner is ready.'}
      </p>
    </div>
  );
}
